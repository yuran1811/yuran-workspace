menu(type='*' where=window.is_taskbar||sel.count mode=mode.multiple title=title.go_to sep=sep.bottom image=\uE14A)
{
	menu(title='Folder' image=\uE1F4)
	{
		item(title='@user.name@@@sys.name' vis=label)
		item(title='Desktop' image=\uE0FB cmd=user.desktop)
		item(title='Downloads' image=\uE0BC cmd=user.downloads)
		item(title='Pictures' image=\uE150 cmd=user.pictures)
		item(title='Documents' image=\uE108 cmd=user.documents)
		separator
